...
public class DynamicTextureTarFlowing extends DynamicTexture {
	private float[] arr1;
	private float[] arr2;
	private float[] arr3;
	private float[] arr4;
	...
	public DynamicTextureTarFlowing(@NonNull IconCoordinate targetTexture) {
		super(targetTexture);
	}
	...
	public void update() {
		...
		for(int i = 0; i < this.targetTexture.getArea(); ++i) {
			float brightness = this.arr1[pmod(i - this.ticks / 3 * this.targetTexture.width, this.targetTexture.getArea())] * 2.0F;
